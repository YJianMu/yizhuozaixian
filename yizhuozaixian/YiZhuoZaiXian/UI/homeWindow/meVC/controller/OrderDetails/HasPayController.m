...
@interface HasPayController ()<UITableViewDataSource,UITableViewDelegate,MerchantInformationHeaderViewDelegate,UIGestureRecognizerDelegate>
...
@implementation HasPayController
...
- (IBAction)goToPay:(id)sender {
    ...
    __weak HasPayController *weakself = self;
   [manager POST:sureorderURL parameters:param success:^(AFHTTPRequestOperation *operation, id responseObject) {
       id json = [NSJSONSerialization JSONObjectWithData:responseObject options:0 error:nil];
       NSLog(@"%@",json);
       [[NSNotificationCenter defaultCenter] postNotificationName:@"detailsureorder" object:nil userInfo:nil];
...
- (IBAction)cancelOrder:(id)sender {
   RefundViewController  *refound = [RefundViewController new];
    refound.order_sn = _detailModel.order_sn;
